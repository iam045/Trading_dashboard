...
def get_expectancy_data(xls):
    ...
    try:
        ...
        # [關鍵修正步驟 2]：過濾「非交易列」（排除小計行）
        # 邏輯：真正的交易紀錄，「策略 (Strategy)」欄位通常會有值。
        # 如果該行有損益數字，但「策略」是空的，極高機率是 Excel 的「當日小計」行。
...
def display_expectancy_lab(xls):
    ...
    if len(unique_months) > 0:
        ...
        # [新增功能] 每日明細除錯工具
        # 讓你直接看到程式到底把哪些列歸類在 12/9，找出誰是多餘的
        with st.expander("🕵️‍♂️ 每日資料檢查器 (Debug)", expanded=False):
            check_date = st.date_input("選擇要檢查的日期", value=pd.Timestamp(y, m, 9))
            check_str = check_date.strftime('%Y-%m-%d')
            debug_data = df[df['DateStr'] == check_str][['DateStr', 'Strategy', 'PnL', 'Risk_Amount']]
            st.write(f"日期 **{check_str}** 的所有原始交易資料：")
            st.dataframe(debug_data)
            st.write(f"程式計算總和: **${debug_data['PnL'].sum():,.0f}**")
