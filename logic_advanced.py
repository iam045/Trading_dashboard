...
def plot_symbol_ranking(df):
    ...
    # 抓取虧損前 5 名 (頭部) 與 獲利前 5 名 (尾部)
    top_losses = symbol_stats.head(5)
    top_profits = symbol_stats.tail(5)
    
    # 合併數據並移除重複(避免標的太少時重複抓取)，再次排序
    df_rank = pd.concat([top_losses, top_profits]).drop_duplicates().sort_values('PnL', ascending=True)
...
def plot_strategy_performance(df):
    ...
    fig = go.Figure()
    fig.add_trace(go.Bar(x=stats['Strategy'], y=stats['Total_PnL'], marker_color=colors, text=stats['Total_PnL'].apply(lambda x: f"${x:,.0f}"), textposition='auto', name='總損益'))
    ...
    fig.update_layout(title="策略總損益與勝率", yaxis=dict(title="總損益 ($)"), yaxis2=dict(title="勝率 (%)", overlaying='y', side='right', tickformat='.0%'), showlegend=True, height=350, margin=dict(t=40, b=20, l=40, r=40))
    return fig
...
def plot_cumulative_pnl_by_strategy(df):
    ...
    fig.update_layout(height=350, hovermode="x unified", margin=dict(t=40, b=20, l=20, r=20))
    return fig
...
def plot_strategy_quality_bubble(df):
    ...
    fig = px.scatter(stats, x="Win_Rate", y="Payoff_Ratio_R", size="Bubble_Size", color="Total_PnL", hover_name="Strategy", color_continuous_scale=["#26a69a", "#eeeeee", "#ef5350"], title="策略品質矩陣 (R)")
